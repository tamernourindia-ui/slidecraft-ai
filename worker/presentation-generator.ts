...
async function generatePresentationVersion(slides: SlideData[], settings: BackendGenerationSettings): Promise<Blob> {
  ...
  // Explicitly cast to Promise<any> then Promise<Blob> for worker compatibility
  return pres.write('blob') as Promise<any> as Promise<Blob>;
}
...
${slideData.content}
Key Talking Points:
${slideData.key_points.map((p) => `• ${p}`).join('\n')}
---
